 ...
 * 
 * Usage:
 *   node check-docs-updated.js [--base <branch>]
 * 
 * Options:
...
const c = (color, text) => useColors ? `${colors[color]}${text}${colors.reset}` : text;
...
function getChangedFiles(baseBranch) {
  try {
    ...
    console.error(c('yellow', `Warning: Could not get changed files: ${error.message}`));
    return [];
  }
}
...
function isSignificantFile(file) {
  // Check exact matches
