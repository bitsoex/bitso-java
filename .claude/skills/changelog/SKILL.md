...
name: changelog
description: >
  Enforce Common Changelog format with Unreleased section extension.
  Validates changelog on commits, generates from git history, and ensures
  all merged PRs are documented. Use when setting up changelog workflows
...
**📚 references/** - [common changelog spec](references/common-changelog-spec.md), [unreleased extension](references/unreleased-extension.md), [migration guide](references/migration-guide.md), [hallmark analysis](references/hallmark-feature-analysis.md)

**🔧 scripts/** - [validate](scripts/validate.ts), [check updated](scripts/check-updated.ts), [coverage](scripts/coverage.ts), [retrofit](scripts/retrofit.ts), [parse](scripts/parse.ts), [migrate](scripts/migrate.ts), [import releases](scripts/import-releases.ts)

**📦 assets/** - [changelog template](assets/changelog-template.md), [exclusion patterns](assets/exclusion-patterns.json)

### Sections

[Quick Start](#quick-start) | [Format Specification](#format-specification) | [Validation Rules](#validation-rules) | [Scripts](#scripts) | [Migration](#migration) | [Integration](#integration) | [Exclusions](#exclusions) | [Troubleshooting](#troubleshooting)
...
| Skill | Purpose |
|-------|---------|
| `quality-gateway` | Orchestrates quality checks including changelog |
| `git-hooks` | Hook infrastructure for pre-commit/pre-push |
| `pr-lifecycle` | PR creation and management |
...
<!-- Source: bitsoex/ai-code-instructions → global/skills/changelog/SKILL.md -->
